...
@param 
    ...
    sizes: int[]
        array of sizes, [0] is width [1] is height of screen
'''
def draw_target(pen, sizes):
    pen.color("pink")
    pen.penup()
    pen.right(90)
    pen.forward(sizes[0])
...
@returns bool 
    true if shot hit
    false if shot didn't hit
'''
def shoot_dart(pen, sizes, color) -> bool:
    hit = False
...
def main():
    ...
    draw_target(pen, sizes)
    for i in range(40):
        shoot_dart(pen, sizes, "green")
